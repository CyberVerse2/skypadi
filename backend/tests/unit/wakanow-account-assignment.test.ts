...
test("Wakanow account assignment persists one account per booking", async () => {
  ...
  assert.match(sqlText, /insert into skypadi_whatsapp\.supplier_account_assignments/);
  assert.match(sqlText, /on conflict \(booking_id, supplier\) do nothing/);
  assert.match(sqlText, /pg_advisory_xact_lock/);
  assert.match(sqlText, /count\(\*\)::int %/);
});
...
test("Wakanow account assignment fails when persisted account is no longer configured", async () => {
  ...
  await assert.rejects(
    ...
    /Assigned Wakanow account is not configured/,
  );
});
