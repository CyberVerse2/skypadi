...
function getKey(): Buffer {
  ...
  // Accept hex (64 chars) or base64 (44 chars with padding). Fall back to base64.
  const hex = /^[0-9a-fA-F]{64}$/.test(raw) ? Buffer.from(raw, "hex") : Buffer.from(raw, "base64");
  if (hex.length !== KEY_LEN) {
    throw new Error(`WALLET_ENCRYPTION_KEY must decode to ${KEY_LEN} bytes (got ${hex.length})`);
  }
  return hex;
}
