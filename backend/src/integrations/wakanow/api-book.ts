...
async function handlePostSubmitModal(
  ...
  if (needsVerification) {
    ...
    const verification = await resolveVerificationCode(request, customerEmail, passenger, notify);
    verificationStatus = verification.status;
    const { code, consume } = verification;
    if (!code) {
      const err: any = new Error("Verification code required but no Resend OTP was available for this booking alias");
      err.debugScreenshots = debugScreenshots;
      throw err;
    }
    console.log(`[api-book] Entering verification code from Resend inbound email`);
    ...
    if (verifyBtn) {
      ...
      if (clicked) await waitForSubmitOutcome(page);
    }
...
async function resolveVerificationCode(
  ...
  passenger: Passenger,
  notify: (msg: string) => Promise<void>
): Promise<{ code?: string; consume?: () => Promise<void>; status: BookingVerificationStatus }> {
  if (!request.resolveOtp) return { status: "not_needed" };

  await notify("⏳ Waiting for booking verification email...");
  ...
  if (!otp?.code) return { status: "manual_assist" };
  await notify("⏳ Verification code received. Continuing your booking...");
  return { code: otp.code, consume: otp.consume, status: "automated" };
}
